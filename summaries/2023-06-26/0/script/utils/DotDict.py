...
class DotDict(dict):
    ...
    def numpy(self):
        """
        Numpy all items of `DotDict` object.
        """
    ...
    # def dotdict2tensordict func
    @staticmethod
    def dotdict2tensordict(dotdict):
        """
        Convert `DotDict` object to `tf.Tensor`.
        Not change the original dotdict, generate new object.
        :param dotdict: The `DotDict` of list, each of which has the same shape.
        :return tensordict: The `DotDict` of list, each of which is `tf.Tensor`.
        """
        tensordict = DotDict()
        keys = DotDict.iter_keys(dotdict)
        for key in keys:
            try:
                DotDict.iter_setattr(tensordict, key, tf.constant(DotDict.iter_getattr(dotdict, key)))
            except Exception:
                DotDict.iter_setattr(tensordict, key, DotDict.iter_getattr(dotdict, key))
        # Return the final tensordict.
        return tensordict

    # def tensordict2numpydict func
    @staticmethod
    def tensordict2numpydict(tensordict):
        """
        Convert `tf.Tensor` to `DotDict` object.
        Not change the original dotdict, generate new object.
        :param tensordict: The `DotDict` of list, each of which is `tf.Tensor`.
        :return dotdict: The `DotDict` of list, each of which is `np.array`.
        """
        dotdict = DotDict()
        keys = DotDict.iter_keys(tensordict)
        for key in keys:
            try:
                DotDict.iter_setattr(dotdict, key, DotDict.iter_getattr(tensordict, key).numpy())
            except Exception:
...
if __name__ == "__main__":
    ...
    # Numpy the `DotDict` object.
    dotdict_inst = dotdict_inst.numpy()
    # dotdict2dict.
    ...
    # Convert dotdict to tensordict.
    tensordict = DotDict.dotdict2tensordict(dotdict)
    # Convert tensordict to numpydict.
    numpydict = DotDict.tensordict2numpydict(tensordict)
